...
public class MovieController : ControllerBase
{
    ...
    public async Task<IActionResult> AddMovie([FromBody] Movie movie)
    {
        try
        {
            ...
            {
                return BadRequest("Film o takich samych danych ju¿ istnieje.");
            }
    ...
    public async Task<IActionResult> AddMovies([FromBody] List<Movie> movies)
    {
        ...
        {
            return BadRequest("Movie list is empty or null.");
        }
    ...
    public async Task<IActionResult> UpdateMovie(int id, [FromBody] Movie movie)
    {
        if (id != movie.Id)
        {
            return BadRequest("The movie ID in the URL does not match the ID in the body.");
        }

        if (movie == null || movie.Year == 0)
        {
            return BadRequest("Movie data is null or the year is invalid.");
        }
        ...
        {
            return Conflict("A movie with the same details already exists.");
        }
    ...
    public async Task<IActionResult> FetchExternalMovies()
    {
        try
        {
            ...
            {
                return BadRequest($"Failed to fetch movies from external API, StatusCode: {response.StatusCode}");
            }
            ...
            {
                return BadRequest("No movies fetched from external API.");
            }
